...
from tools.analytics import get_spending_by_category, get_cash_flow
...
from schemas.customer_report import CustomerReport, ReportMeta, SalaryBlock, EMIBlock, BillBlock, RentBlock
...
def build_customer_report(customer_id: int, months: int = 6,
                          rg_salary_data: Optional[dict] = None) -> CustomerReport:
    """
    ...
    meta = ReportMeta(customer_id=customer_id, prty_name=prty_name, generated_at=datetime.now().isoformat(), # 3. Build report meta
                     analysis_period=f"Last {months} months", currency="INR",transaction_count=transaction_count)

    
    category_overview = safe_call(_get_category_overview, customer_id) # 4. Get category overview
    monthly_cashflow = safe_call(_get_monthly_cashflow, customer_id) # 5. Get monthly cashflow
    summary = safe_call(fetch_transaction_summary, customer_id) # 6. Fetch the transaction summary
    top_merchants = safe_call(_get_top_merchants, summary) # 7. Get top merchants
    salary_block = safe_call(_get_salary_block, summary, cust_df) # 8. Get salary block
    emis = safe_call(_get_emi_block, cust_df) # 9. Get EMI block
    rent_block = safe_call(_get_rent_block, customer_id) # 10. Get rent block
    bills = safe_call(_get_bills_block, customer_id) # 11. Get bills block
    merchant_features = safe_call(compute_all_merchant_features, customer_id) or None # 12. Get merchant-level behavioral features
    ...
    # 13. Compute account quality & Detect semantic events
    account_quality = safe_call(compute_account_quality, customer_id, customer_report=base_report, rg_salary_data=rg_salary_data)
    events = safe_call(detect_events, customer_id, rg_salary_data=rg_salary_data) or None 

    # 15. Augment with L2-tagged loan disbursals that the keyword rules missed.
    events = safe_call(_augment_disbursal_events_from_l2, cust_df, events or [], default=events)

    updates = {}
    if account_quality: updates["account_quality"] = account_quality
    if events: updates["events"] = events

    final_report = base_report.model_copy(update=updates) if updates else base_report

    # 14. Compute deterministic review checklist
    ...
    if checklist: final_report = final_report.model_copy(update={"checklist": checklist})
...
def _get_monthly_cashflow(customer_id: int) -> Optional[list]:
    ...
    cashflow_list = [{"month": k, "inflow": v.get('inflow', 0), "outflow": v.get('outflow', 0), "net": v.get('net', 0)}
                    for k, v in sorted(monthly_data.items())] # Convert to list format for template
...
def _get_top_merchants(summary) -> Optional[list]:
    ...
    """
    if not summary or not summary.high_frequency_transactions: return None
    ...
    top_merchants = [
        {"name": t.representative_narration, "count": t.count, "total": t.total_amount, "avg": t.average_amount, 
        "type": t.transaction_type, "score": t.score, "similar_narrations": list(t.similar_narrations or [])}
        for t in top
    ]
...
def _get_salary_block(summary, cust_df) -> Optional[SalaryBlock]:
    ...
    if not summary or not summary.salary_summary: return None
    salary = summary.salary_summary

    latest_transaction, all_salary_dates = _salary_scan(cust_df) # Latest salary transaction + all occurrence dates in a single scan.
    return SalaryBlock(avg_amount=salary.average_amount, frequency=salary.transaction_count,
                       narration=salary.narrations[0] if salary.narrations else "",
                       sample_transaction={"amount": salary.average_amount, "total": salary.total_amount}, 
                       latest_transaction=latest_transaction, dates=all_salary_dates)
...
def _salary_scan(cust_df) -> Tuple[Optional[dict], list]:
    ...
    try:
        ...
        for _, row in cust_df.iterrows():
            if row.get('dr_cr_indctor') != 'C': continue
...
def _augment_disbursal_events_from_l2(cust_df, events: list) -> list:
    ...
    if cust_df.empty or 'category_of_txn_l2' not in cust_df.columns: return events

    l2_mask = cust_df['category_of_txn_l2'].apply(lambda v: l2_canonical(v) == 'Loan_Disburse')
    ...
    for ev in events:
        if ev.get('type') == 'loan_disbursal':
            existing_keys.add((str(ev.get('date') or ''), round(float(ev.get('amount') or 0), 0)))
...
def _get_emi_block(cust_df) -> Optional[list]:
    ...
    from tools.category.registry import has_role
    from utils.narration_utils import extract_recipient_name, clean_narration, exact_then_fuzzy_group

    cust_df = cust_df[cust_df['dr_cr_indctor'] == 'D'].copy()
    if cust_df.empty: return None
    ...
    if 'category_of_txn_l2' in cust_df.columns: l2_mask = cust_df['category_of_txn_l2'].apply(lambda v: has_role(v, 'emi'))
    else: l2_mask = narr_mask & False
    ...
    for _, row in matched.iterrows():
        ...
        if not name: name = clean_narration(narration) or 'EMI Payment'
        rows.append({
            'name': name,
            'narration': narration,
            'date': str(row.get('tran_date', '')),
    ...
    for r in rows:
        ...
        d = r['date'].strip()
        if d: dates_by_name[name].append(d)
...
def _presence_stats(customer_id: int, category: str):
    ...
    if not result.get('present'): return None
...
def _get_rent_block(customer_id: int) -> Optional[RentBlock]:
    ...
    return RentBlock(direction="paid", frequency=txn_count, amount=avg_amount, sample_transaction=sample, dates=dates)
...
def _get_bills_block(customer_id: int) -> Optional[list]:
    ...
    if not stats: return None
    txn_count, avg_amount, sample, dates = stats
    return [BillBlock(bill_type="Utilities", frequency=txn_count, avg_amount=avg_amount, sample_transaction=sample, dates=dates)]
